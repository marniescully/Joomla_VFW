...
var MultiUpload = new Class(
{
	 ...
	 */
	initialize:function( input_element, max, name_suffix_template, show_filename_only, remove_empty_element, delete_file ){
		...
		// This is optional
		if( name_suffix_template != undefined ){
		...
		// This is optional
		if( show_filename_only != undefined ){
		...
		$(container).inject( input_element, 'after' );
	...
	addRow:function(){
		if( this.max == 0 || this.elements.length <= this.max ){
			...
			new_input.inject( current_element.element, 'after');
		} else {
